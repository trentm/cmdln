#!/usr/bin/env python
#
# Experimental re-write of cmdtest.py

"""
    Test getting Python's version.
...
#TODO: class _OutputRedirectingPdb(pdb.Pdb):
...
## 2. Example & CmdTest
######################################################################
## - An "example" is a <input, want> pair, where "input" is a
##   command line, and "want" is the expected output for
##   "input."  The Example class also includes information about
...
class Example:
    """
    A single cmdtest example, consisting of command line and expected
...
class CmdTest:
    ...
    namespace.  Each `CmdTest` defines the following attributes:
    ...
    def __init__(self, examples, globs, name, filename, lineno, docstring):
        """
        Create a new CmdTest containing the given examples.  The
        CmdTest's globals are initialized with a copy of `globs`.
        """
        assert not isinstance(examples, basestring), \
               "CmdTest doesn't accept str; use CmdTestParser instead"
...
class CmdTestParser:
    ...
    def get_cmdtest(self, string, globs, name, filename, lineno):
        """
        ...
        """
        return CmdTest(self.get_examples(string, name), globs,
...
class CmdTestFinder:
    ...
    objects.  Cmdtests can currently be extracted from the following
    ...
    def find(self, obj, name=None, module=None, globs=None, extraglobs=None):
        ...
        correct module.  The object's module is used:
            ...
            - To prevent the CmdTestFinder from extracting CmdTests
        ...
        objects will (recursively) be searched for cmdtests.

        The globals for each CmdTest is formed by combining `globs`
        and `extraglobs` (bindings in `extraglobs` override bindings
        in `globs`).  A new copy of the globals dictionary is created
        for each CmdTest.  If `globs` is not specified, then it
        ...
        if name is None:
            ...
            if name is None:
                raise ValueError("CmdTestFinder.find: name must be given "
        ...
        # Recursively expore `obj`, extracting CmdTests.
        tests = []
        self._find(tests, obj, name, module, source_lines, globs, {})
        # Sort the tests by alpha order of names, for consistency in
        # verbose-mode output.  This was a feature of cmdtest in Pythons
    ...
    def _get_test(self, obj, name, module, globs, source_lines):
        ...
        if module is None:
            ...
            if filename[-4:] in (".pyc", ".pyo"):
                filename = filename[:-1]
        return self._parser.get_cmdtest(docstring, globs, name,
                                        filename, lineno)
...
class CmdTestRunner:
    ...
    tried, and `f` is the number of test cases that failed.
        ...
        >>> tests = CmdTestFinder().find(_TestClass)
...
#        self._fakeout = _SpoofOut()
    ...
    def __run(self, test, compileflags, out):
        ...
        for examplenum, example in enumerate(test.examples):
            ...
            filename = '<cmdtest %s[%d]>' % (test.name, examplenum)
    ...
    __LINECACHE_FILENAME_RE = re.compile(r'<cmdtest '
                                         r'(?P<name>[\w\.]+)'
                                         r'\[(?P<examplenum>\d+)\]>$')
...
class OutputChecker:
    """
    A class used to check the whether the actual output from a cmdtest
...
class CmdTestFailure(Exception):
...
class DebugRunner(CmdTestRunner):
    r"""Run cmd tests but raise an exception as soon as there is a failure.
